	...
	if(is_null($layout)) {
		if(isset($_GET['view'])) {
			...
			if(@include_once($path)) {
				...
				die("Vue ".$_GET['view']." indisponible!");
			}
		}
	} else {
		if(isset($_GET['layout'])) {
			//$path = $_SERVER['DOCUMENT_ROOT'].DS.'views'.DS.$_GET['view'].'.php';
			$path = realpath('.'.DS.'views'.DS.$_GET['layout'].'.php');
			//var_dump($path);
			if(@include_once($path)) {
				
			} else {
				die("Layout ".$_GET['layout']." indisponible!");
			}
		}
	}
}
...
function dateMysqlEn($date1, $date2 = null) {
	$tab = array();
	$tab = explode('/', $date1);
	$date2 = '';
	$date2 .= $tab[2] . "-" . $tab[0] . "-" . $tab[1];
	return substr($date2, 0, strlen(rtrim($date2)));	
}
...
$getHeaderComponent = function ($fichier) {
	include_once('components'.DS.$fichier.'.php');
};

$getLeftComponent = function ($fichier) {
	include_once('components'.DS.$fichier.'.php');
};
$getContent = function ($fichier) {
	include_once('views'.DS.$fichier.'.php');
};
